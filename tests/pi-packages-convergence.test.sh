  ...
  list)
    ...
    if [ -f "$STATE/sources" ]; then
      while IFS= read -r source_spec; do
        ...
        if [ "${FILTERED_SOURCE:-}" = "$source_spec" ]; then
          printf '  %s (filtered)\n' "$source_spec"
        else
          printf '  %s\n' "$source_spec"
        fi
...
run_converger() {
  PATH="$runtime_bin" CALLS="$calls" STATE="$state" \
    PI_VERSION="${PI_VERSION:-0.84.3}" FILTERED_SOURCE="${FILTERED_SOURCE:-}" \
...
printf '%s\n' 'captain web config' > "$agent_dir/web-search.json"
...
: > "$calls"
FILTERED_SOURCE='npm:pi-web-access@0.25.0' run_converger
...
[ -e "$agent_dir/web-search.json" ] || fail 'web-search config disappeared during fresh convergence'
...
# A repeat activation is a no-op, including when Pi displays one package as a
# filtered package entry.
FILTERED_SOURCE='npm:pi-web-access@0.25.0' run_converger
assert_eq "$expected_calls" "$(cat "$calls")"

# Stale configured versions and refs are repaired to the reviewed pins. The
# unrelated package remains configured and no remove operation is available to
# the helper, proving preservation is part of the observable contract.
...
[ -e "$agent_dir/web-search.json" ] || fail 'web-search config was changed during stale repair'
...
if PATH="$runtime_bin" CALLS="$bad_calls" STATE="$bad_state" PI_VERSION='0.84.4' FILTERED_SOURCE='' \
...
printf 'ok - fresh install, idempotent repeat, stale pin repair, unrelated-package preservation, restricted PATH, and incompatible-version refusal\n'
