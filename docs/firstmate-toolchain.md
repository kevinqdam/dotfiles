...
Mechanical fully specified edits, well-scoped implementation, no-mistakes, validation, CI, unattended pipelines, and the unpinned execution default route to Pi `openai-codex/gpt-6-luna` at effort `max`. Luna max owns this standing execution route; the independent Firstmate finished-output look remains Sol high by default. Do not omit it to save quota. Astra is not reserved by a plan or review stage and does not implement, test, or watch CI.
Explicit captain per-task `--harness`, `--model`, and `--effort` requests take precedence, including an explicit Astra choice or alternate Pi route selection. No automatic second Sol or Astra loop and no implicit merge permission.
...
A seed model change does not migrate an existing home. Rebuild leaves an existing regular `crew-dispatch.json` byte-for-byte unchanged, including an older concrete model ID or an already selected successor. It does not link, seed, or overwrite captain memory. The global Pi model selection in `~/.pi/agent/settings.json` is unrelated captain-owned runtime state. Activation does not read or replace it, and a seed change is not authority to overwrite it.
...
- `agent: pi`
- a fresh/default `agent_args_override.pi`: `--model openai-codex/gpt-6-luna --thinking max` only when the Pi node is absent

The `agent: pi` key is enforced for test, lint, push, and PR; it is not `auto`, which would hire Codex because Codex is installed. Existing operator-selected Pi arguments, including provider, model, thinking, extra flags, comments, and an explicit empty list, are preserved verbatim. An existing Pi node that still names an older model is also preserved. Activation does not string-replace a historical model ID, because that cannot distinguish an explicit pin from a previous seed. Fresh/missing defaults use Luna at max; existing Grok, Luna 5.6, and custom or empty Pi nodes remain untouched. Grok Bot.app is not this agent, and the grok CLI is not installed; Firstmate retains Grok medium for coordinator interviews, routing, and context packing, while Luna max remains the standing execution default.
...
An existing-home migration, if separately authorized, is not seed convergence. Existing Firstmate dispatch and no-mistakes Pi argument nodes, including seed-shaped Grok 4.7 and Luna 5.6 values, remain byte-for-byte unchanged; do not infer that a concrete value is still an uncustomized seed. Any future migration requires a separately scoped inspect-then-update while the daemon is idle. A disk edit does not prove the running daemon has loaded new arguments. Activation must not stop, restart, or update the shared no-mistakes daemon; if a restart is ever required, Firstmate arranges it only after every lane's current run has finished and confirms the next safely started run.
...
Never omit the finished-output review to save quota. After that independent look, Luna max drives no-mistakes with `--skip=review`; do not skip validation. Keep the no-mistakes agent Pi and preserve captain-selected model/effort arguments. The standing Luna default applies to fresh execution, and explicit captain selections still take precedence.
...
The same file tells the coordinator to adopt the newest verified generally available successor in the chosen Grok family for unpinned coordinator Grok-medium defaults, without a fresh upgrade request, using Firstmate's existing catalog-aware selection policy. This applies only to the coordinator's Grok-medium role of interviewing, routing, and context packing, not to execution: the separate standing execution default is Luna at max. It preserves explicit pins and configured roles and efforts, and it forbids interrupting active sessions or runs. Workers do not apply the successor rule. The managed text is available after rebuild and a Pi context reload or new session. Do not write through the live `~/.pi/agent/AGENTS.md` link. Prompt-composition tests prove that delivery only. They do not prove model obedience or scheduled automation.
...
Rebuild preserves an existing `~/.local/share/firstmate/config/crew-dispatch.json` byte-for-byte, including Astra-shaped routing, historical Grok or Luna model IDs, effort pins, and unrelated captain choices. An old Astra dispatch beside the updated global Sol-first policy is a known mismatch, not a live migration.
...
Fresh or missing Firstmate and no-mistakes execution defaults use Pi `openai-codex/gpt-6-luna` at `max`. Existing Firstmate dispatch and no-mistakes Pi argument nodes are not migrated by rebuild, including historical Grok and Luna 5.6 pins or explicit non-max effort. Do not change Pi global settings. This source change does not itself modify any live Firstmate/no-mistakes config, credentials, or user settings.
...
Isolated prompt-composition fixtures and development-only model checks live in `tests/firstmate-policy.test.sh` and `tests/firstmate-policy-model-checks.sh`. They do not replace live-home rebuild and reload, and they do not prove model obedience or scheduled automation. The optional prompt-check script pins Grok 4.7 at medium as its isolated policy-probe model; that probe is not the standing execution default. `tests/firstmate-policy-model-check-evidence.md` is historical evidence and stays unchanged unless those checks are actually rerun.
